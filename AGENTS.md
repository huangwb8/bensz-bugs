...
本项目用于 用于公开收集与管理 Bensz 系列 Agent Skills 在真实用户环境中暴露出的设计缺陷类 bug。。

## 项目目标

用于公开收集与管理 Bensz 系列 Agent Skills 在真实用户环境中暴露出的设计缺陷类 bug。

本工作区的核心目标是 用于公开收集与管理 Bensz 系列 Agent Skills 在真实用户环境中暴露出的设计缺陷类 bug。。

## 核心工作流

当用户提出 通用项目 相关需求时，按以下流程执行：
...
project_info:
  ...
  description: "用于公开收集与管理 Bensz 系列 Agent Skills 在真实用户环境中暴露出的设计缺陷类 bug。"
```
